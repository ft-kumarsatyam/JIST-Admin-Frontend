# Contributing to JIST

Thank you for showing an interest in contributing to JIST! All kinds of contributions are valuable to us. In this guide, we will cover how you can quickly onboard and make your first contribution.
...
- Postgres 15 (Nest via `plane-backend` docker compose on `:5433`)
- **Memory**: Minimum **8 GB RAM** recommended for FE monorepo builds

### Setup the project

Frontend is this monorepo (`web` / `admin` / `space`). Backend is **NestJS** in the sibling `plane-backend` repo — not Django `apps/api`.

1. Clone the repos

```bash
git clone https://github.com/ft-kumarsatyam/khelo-tech-PMM.git plane-frontend
git clone https://github.com/ft-kumarsatyam/khelo-tech-PMM-Backend.git plane-backend
cd plane-frontend
chmod +x setup.sh
```
...
5. Open your browser to http://localhost:3001/admin/ and register yourself as instance admin
...
- Try JIST Cloud and the self hosting platform and give feedback
...
- Report a bug
- **Improve documentation** - fix incomplete or missing [docs](https://www.khelo.tech/), bad wording, examples or explanations.
...
Questions, suggestions, and thoughts are most welcome. We can also be reached in our [Forum](https://www.khelo.tech).
