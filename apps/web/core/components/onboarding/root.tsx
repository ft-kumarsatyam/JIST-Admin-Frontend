    ...
    (step: EOnboardingSteps, skipInvites?: boolean) => {
      switch (step) {
        ...
        case EOnboardingSteps.ROLE_SETUP:
          setCurrentStep(EOnboardingSteps.BITBUCKET_CONNECT);
          break;
        case EOnboardingSteps.USE_CASE_SETUP:
          setCurrentStep(EOnboardingSteps.BITBUCKET_CONNECT);
          break;
        case EOnboardingSteps.BITBUCKET_CONNECT:
          stepChange({ profile_complete: true, bitbucket_connect: true });
  ...
  useEffect(() => {
    const handleInitialStep = () => {
      if (
        ...
        userProfile?.onboarding_step?.bitbucket_connect &&
