...
export const AuthPasswordForm = observer(function AuthPasswordForm(props: Props) {
  const { email, isSMTPConfigured, handleAuthStep, handleEmailClear, mode, nextPath, invitationId, workspaceSlug } =
    props;
  // plane imports
  ...
  return (
    <>
      ...
      >
        ...
        {invitationId && <input type="hidden" value={invitationId} name="invitation_id" />}
        {workspaceSlug && <input type="hidden" value={workspaceSlug} name="slug" />}
        <div className="space-y-1">
          <label htmlFor="email" className="text-13 font-medium text-[#3F2271]">
            {t("auth.common.email.label")}
          </label>
          <div className={`relative flex items-center rounded-md border border-[#D6C8EC] bg-white`}>
            <Input
              id="email"
              name="email"
              type="email"
              ...
              className={`h-10 w-full border-0 bg-white text-[#1A102B] disable-autofill-style placeholder:text-[#7A6B94]`}
              disabled
            />
            {passwordFormData.email.length > 0 && (
              <button
                type="button"
                className="absolute right-3 size-5 text-[#5B4B72] hover:text-[#1A102B]"
        ...
        <div className="space-y-1">
          <label htmlFor="password" className="text-13 font-medium text-[#3F2271]">
          ...
          <div className="relative flex items-center rounded-md bg-white">
            <Input
              ...
              className="h-10 w-full border border-[#D6C8EC] !bg-white pr-12 text-[#1A102B] disable-autofill-style placeholder:text-[#7A6B94]"
            ...
            <button
              ...
              className="absolute right-3 grid size-5 place-items-center text-[#5B4B72] hover:text-[#1A102B]"
              aria-label={t(
                showPassword?.password ? "aria_labels.auth_forms.hide_password" : "aria_labels.auth_forms.show_password"
              )}
            >
              {showPassword?.password ? <EyeOff className="size-5" /> : <Eye className="size-5" />}
            </button>
          </div>
          {passwordSupport}
        </div>
        ...
        {mode === EAuthModes.SIGN_UP && (
          <div className="space-y-1">
            <label htmlFor="confirm-password" className="text-13 font-medium text-[#3F2271]">
            ...
            <div className="relative flex items-center rounded-md bg-white">
              ...
              >
                {showPassword?.retypePassword ? <EyeOff className="size-5" /> : <Eye className="size-5" />}
              </button>
            </div>
            {!!passwordFormData.confirm_password &&
              passwordFormData.password !== passwordFormData.confirm_password &&
              renderPasswordMatchError && (
                <span className="text-13 text-[#B42318]">{t("auth.common.password.errors.match")}</span>
              )}
          </div>
        )}
