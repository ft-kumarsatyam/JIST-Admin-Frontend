...
const Titles = {
  [EAuthModes.SIGN_IN]: {
    ...
    [EAuthSteps.UNIQUE_CODE]: {
      ...
      subHeader: "Enter the code we sent you.",
    },
  },
  [EAuthModes.SIGN_UP]: {
    [EAuthSteps.EMAIL]: {
      header: "Create your JIST account",
      subHeader: "Start in a couple of minutes.",
    },
    [EAuthSteps.PASSWORD]: {
      header: "Set a password",
      subHeader: "Choose a strong password to protect your account.",
    },
    [EAuthSteps.UNIQUE_CODE]: {
...
export const AuthHeader = observer(function AuthHeader(props: TAuthHeader) {
  ...
  const { data: invitationData, isLoading } = useSWR(
  ...
  const getHeaderSubHeader = (
    ...
    if (memberInvitation && inviteEmail && memberInvitation.email === inviteEmail && memberInvitation.workspace) {
...
export function AuthHeaderBase(props: TAuthHeaderBase) {
  return (
    <div className="flex flex-col gap-1.5">
      <h1 className="text-[22px] font-semibold tracking-[-0.02em] text-[#1A102B]">{props.header}</h1>
      <p className="text-[14px] leading-5 font-medium text-[#5B4B72]">{props.subHeader}</p>
    </div>
  );
}
