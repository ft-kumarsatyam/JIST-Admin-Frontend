...
export const coreRoutes: RouteConfigEntry[] = [
  ...
  // Home → redirect to user sign-in
  layout("./(home)/layout.tsx", [index("./(home)/page.tsx")]),

  // User auth (workspace members) — separate from admin app (:3001/admin)
    ...
    layout("./(all)/[workspaceSlug]/layout.tsx", [
      ...
      layout("./(all)/[workspaceSlug]/(settings)/layout.tsx", [
        ...
        layout("./(all)/[workspaceSlug]/(settings)/settings/(workspace)/layout.tsx", [
          ...
          route(
            ":workspaceSlug/settings/engineering",
            "./(all)/[workspaceSlug]/(settings)/settings/(workspace)/engineering/page.tsx"
          ),
        ]),
