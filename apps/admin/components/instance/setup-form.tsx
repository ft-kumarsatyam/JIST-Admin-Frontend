...
export function InstanceSetupForm({ isFirstSetup = true }: { isFirstSetup?: boolean }) {
  // search params
  ...
  return (
    <>
      ...
      <div className="mt-10 flex w-full flex-grow flex-col items-center justify-center py-6">
        <div className="relative flex w-full max-w-[28rem] flex-col gap-6 rounded-2xl border border-[#E8DFF8] bg-white px-6 py-8 text-[#1A102B] shadow-[0_18px_50px_-28px_rgba(63,34,113,0.35)] sm:px-8">
          <FormHeader
            heading={isFirstSetup ? "Setup your JIST Instance" : "Create an admin account"}
            subHeading={
              isFirstSetup
                ? "Post setup you will be able to manage this JIST instance."
                : "Create a new instance admin to manage JIST."
            }
          />
          {errorData.type &&
            errorData?.message &&
          ...
          >
            ...
            <div className="flex flex-col items-center gap-4 sm:flex-row">
              <div className="w-full space-y-1">
                <label className="text-13 font-medium text-[#3F2271]" htmlFor="first_name">
                  First name <span className="text-[#B42318]">*</span>
                </label>
                <Input
                  className="w-full border border-[#D6C8EC] !bg-white text-[#1A102B] placeholder:text-[#7A6B94]"
                  id="first_name"
                  name="first_name"
                  type="text"
                  inputSize="md"
              ...
              <div className="w-full space-y-1">
                <label className="text-13 font-medium text-[#3F2271]" htmlFor="last_name">
            ...
            <div className="w-full space-y-1">
              <label className="text-13 font-medium text-[#3F2271]" htmlFor="email">
              ...
              {errorData.type && errorData.type === EErrorCodes.INVALID_EMAIL && errorData.message && (
                <p className="px-1 text-11 text-[#B42318]">{errorData.message}</p>
              )}
            </div>

            <div className="w-full space-y-1">
              <label className="text-13 font-medium text-[#3F2271]" htmlFor="company_name">
                Company name <span className="text-[#B42318]">*</span>
              </label>
              <Input
                className="w-full border border-[#D6C8EC] !bg-white text-[#1A102B] placeholder:text-[#7A6B94]"
                id="company_name"
            ...
            <div className="w-full space-y-1">
              <label className="text-13 font-medium text-[#3F2271]" htmlFor="password">
                Set a password <span className="text-[#B42318]">*</span>
              </label>
              <div className="relative">
                <Input
                  className="w-full border border-[#D6C8EC] !bg-white pr-10 text-[#1A102B] placeholder:text-[#7A6B94]"
                  id="password"
                  name="password"
                ...
                {showPassword.password ? (
                  <button
                    ...
                    className="absolute top-3.5 right-3 flex items-center justify-center text-[#5B4B72] hover:text-[#1A102B]"
            ...
            <div className="w-full space-y-1">
              <label className="text-13 font-medium text-[#3F2271]" htmlFor="confirm_password">
              ...
              <div className="relative">
                <Input
                  ...
                  className="w-full border border-[#D6C8EC] !bg-white pr-12 text-[#1A102B] placeholder:text-[#7A6B94]"
              ...
              {!!formData.confirm_password &&
                formData.password !== formData.confirm_password &&
                renderPasswordMatchError && <span className="text-13 text-[#B42318]">Passwords don{"'"}t match</span>}
            </div>
            ...
            <div className="relative flex gap-2">
              ...
              <label className="cursor-pointer text-13 font-medium text-[#5B4B72]" htmlFor="is_telemetry_enabled">
                Allow JIST to anonymously collect usage events.
              </label>
            </div>

            <div className="py-2">
              <Button type="submit" size="xl" className="w-full" disabled={isButtonDisabled}>
                {isSubmitting ? <Spinner height="20px" width="20px" /> : isFirstSetup ? "Continue" : "Create account"}
              </Button>
            </div>
            <p className="text-center text-[13px] font-medium text-[#5B4B72]">
              Already have an account?{" "}
              <Link href="/sign-in/" className="font-semibold text-[#6532B8] hover:text-[#3F2271] hover:underline">
                Sign in
              </Link>
            </p>
          </form>
