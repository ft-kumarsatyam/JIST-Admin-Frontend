> **Khelo / JIST (August 2026):** Default `docker-compose.yml` is **NestJS + Postgres** — no Django, Celery, Gunicorn, or RabbitMQ.
> Legacy Django stack: `docker-compose.django.legacy.yml` (do not use for Khelo).
> Env: `variables.env` (`SESSION_SECRET`, `DATABASE_URL=postgresql://khelo:…`).
...
You have successfully self hosted `JIST` instance. Access the application by going to IP or domain you have configured it (e.g `https://plane.example.com:8080` or `http://[IP-ADDRESS]:8080`)
...
### Upgrading JIST Version

It is always advised to keep JIST up to date with the latest release.
...
1. Make sure that JIST-CE is installed, started, and then stopped. This ensures that the Docker volumes are created.
...
1. Start the JIST instance using `./setup.sh start`.

---

### Restore for Commercial Air-Gapped (Docker Compose)

When you want to restore the previously backed-up data on JIST Commercial Air-Gapped version, follow the instructions below.
...
1. Make sure that JIST Commercial (Airgapped) is extracted and ready to get started. In case it is running, you would need to stop that.
...
1. After restoration, you are ready to start JIST Commercial (Airgapped) will all your previously saved data.
