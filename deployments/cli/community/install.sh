...
function download() {
    ...
    echo ""
    echo "Most recent version of JIST is now available for you to use"
    echo ""
...
function startServices() {
    ...
    if [ -n "$migrator_container_id" ]; then
        ...
        if [ $migrator_exit_code -ne 0 ]; then
            echo "JIST Server failed to start ❌"
            # stopServices
            echo
    ...
    echo "   JIST Server started successfully ✅"
    echo ""
...
function upgrade() {
    ...
    echo "Upgrading JIST to the latest release..."
    echo ""
...
function viewLogs(){
    ...
    then
        ...
        echo "   3) API"
        echo "   4) Worker"
        echo "   5) (retired) Beat-Worker — use Nest api logs"
        echo "   6) Migrator"
        echo "   7) Proxy"
        echo "   8) Redis"
        echo "   9) Postgres"
        ...
        then
            ...
            case $DOCKER_SERVICE_NAME in
                ...
                4) viewSpecificLogs "api"  # was celery worker;;
                5) viewSpecificLogs "api"  # was beat-worker;;
    ...
    then
        ...
        case $ARG_SERVICE_NAME in
            ...
            worker) viewSpecificLogs "api"  # was celery worker;;
            beat-worker) viewSpecificLogs "api"  # was beat-worker;;
